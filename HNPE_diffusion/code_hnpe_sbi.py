...
def main(ctx: mlxp.Context):
    ...
    nn_posterior = inference_alpha.append_simulations(alpha_train, condition, proposal=prior).train(
                #num_atoms=10,
                #learning_rate=1e-4,
    ...
    if cfg.nextra==0:
        ...
        logger.log_metrics({"cov_true":cov_true.tolist(),"cov_est": cov_est.tolist()}, log_name="0_obs")
        ...
        logger.log_metrics({"c2st_alpha_gauss":acc_alpha_gauss.item(),"c2st_beta_gauss":acc_beta_gauss.item(),"c2st_pair":acc_gauss.item()}, log_name=f"{n_obs+1}_obs")
        logger.log_metrics({"c2st_alpha_auto":acc_alpha_auto.item(),"c2st_beta_auto":acc_beta_auto.item(),"c2st_pair":acc_auto.item()}, log_name=f"{n_obs+1}_obs")
        logger.log_metrics({"c2st_alpha_jac":acc_alpha_jac.item(),"c2st_beta_jac":acc_beta_jac.item(),"c2st_pair":acc_jac.item()}, log_name=f"{n_obs+1}_obs")
        logger.log_metrics({"c2st_alpha_fnpe":acc_alpha_fnpe.item(),"c2st_beta_fnpe":acc_beta_fnpe.item(),"c2st_pair":acc_fnpe.item()}, log_name=f"{n_obs+1}_obs")
        ...
        logger.log_metrics({"mmd_alpha_gauss":mmd_alpha_gauss.item(),"mmd_beta_gauss":mmd_beta_gauss.item(),"mmd_pair":mmd_gauss.item()}, log_name=f"{n_obs+1}_obs")
        logger.log_metrics({"mmd_alpha_auto":mmd_alpha_auto.item(),"mmd_beta_auto":mmd_beta_auto.item(),"mmd_pair":mmd_auto.item()}, log_name=f"{n_obs+1}_obs")
        logger.log_metrics({"mmd_alpha_jac":mmd_alpha_jac.item(),"mmd_beta_jac":mmd_beta_jac.item(),"mmd_pair":mmd_jac.item()}, log_name=f"{n_obs+1}_obs")
        logger.log_metrics({"mmd_alpha_fnpe":mmd_alpha_fnpe.item(),"mmd_beta_fnpe":mmd_beta_fnpe.item(),"mmd_pair":mmd_fnpe.item()}, log_name=f"{n_obs+1}_obs")
        ...
        logger.log_metrics({"cov_true":cov_true.tolist(),"cov_est_gauss": cov_est_gauss.tolist(),
                            "cov_est_auto":cov_est_auto.tolist(),
                            "cov_est_jac":cov_est_jac.tolist(),
                            "cov_est_fnpe":cov_est_fnpe.tolist()}, log_name=f"{n_obs+1}_obs")
        ...
        sns.kdeplot(samples_alpha_fnpe.detach().squeeze(1), color="grey", label="Geffner")
        sns.kdeplot(samples_alpha_jac.detach().squeeze(1), color="blue", label="JAC")
        sns.kdeplot(true_samples[:,0], color="black", label="true")
        ...
        sns.kdeplot(true_samples[:,1], color="black", label="true")
