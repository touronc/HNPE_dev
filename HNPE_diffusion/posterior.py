...
class ToyModelFlow_diffusion_nflows(base.Distribution):
    ...
    def __init__(self, batch_theta, batch_x, embedding_net,
        ...
        # a context that is a stacking of the embedded observation x
        # and the gain parameter
        self._embedding_net_2 = embedding_net
       
        flow_2 = build_nsf(batch_x=batch_theta,
                           batch_y=batch_x,
                           z_score_x=z_score_theta,
                           z_score_y=z_score_x,
                           embedding_net=self._embedding_net_2,
                           num_transforms=5)


        self._flow_2 = flow_2 #TYPE: Nflowsflow
        ###ATTENTION
        self.input_shape = batch_theta[0,:].size() #torch.zeros(2,).size()
        #print("input shape",self.input_shape)
        self.condition_shape = batch_x[0,:].size() #torch.zeros((1,1)).size()
        #print("cond shape",self.condition_shape)
        # A DECOMMENTER POUR LANCER UNE VERSION SEQUENTIELLE
        #self.net = self._flow_2.net

    def log_prob(self, inputs, condition):

        logp_2 = self._flow_2.log_prob(inputs, condition)
        #size (1,batch)
    ...
    def sample(self, num_samples, condition):
        ...
        # size (nsamples,2)
        #samples_flow_2 = self._flow_2.sample(num_samples, condition)
        
        # # condition batch passes through a standardizing net then embedding net
        condition_2 = self._flow_2.net._embedding_net(condition) #on va chercher le flow du Nflowflows puis l'embedding net
        #condition_2=condition_2.repeat(num_samples[0],1)
        # condition_2 = self._flow_2.embedding_net(condition_2) #on va chercher le flow du Nflowflows puis l'embedding net
        condition_2 = condition_2.float()
        noise = self._flow_2.net._distribution.sample(num_samples[0])
        samples_flow_2, _ = self._flow_2.net._transform.inverse(noise,context=condition_2)
        return samples_flow_2 
