...
def model(x_obs=None, n_extra=0, eps=0.0001):
    """
...
def plot_true_posterior(true_theta,samples_mcmc):
    ...
    if len(samples_mcmc.keys()) >1: 
        ...
        n_extra = list(samples_mcmc.keys())[0]
        fig, ax = plot.pairplot(samples_mcmc[n_extra], limits=xlim)
        ...
        fig.savefig(fname=f'true_posterior_samples_alpha_{true_theta[1]}_beta_{true_theta[0]}_nextra_{n_extra}.pdf', format='pdf')
...
if __name__ == "__main__":
    ...
    file = "ToyModel_naive_False_ntrials_01_nextra_10_alpha_0.30_beta_0.50_gamma_1.00_noise_0.00_agg_False.csv"
    rng_key = random.PRNGKey(1)
